...
public class TopTagsParserMunicipalities extends HttpServlet implements Runnable {
...
if(countglobal==500){
    ...
    if(coststags[sqlcount]>0){
        int cost=rs.getInt(2);
        int souma=cost+coststags[sqlcount];

        int updateCount2 = stmt3.executeUpdate(
            "UPDATE toptags SET cost ='"+souma+"' WHERE tag='"+tags[sqlcount]+"';");
    }
    sqlcount++;
